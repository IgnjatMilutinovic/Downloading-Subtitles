...
async function subtitles(){
  ...
  const imdbMovie = async () => {
      try {
          const res = await axios.get(`https://imdb-api.com/en/API/SearchTitle/#Enter Your Token Here/${nameAndYear.name}` + ' ' + `${nameAndYear.year}`);
  ...
  let data1 = await OpenSubtitles.api.LogIn(# Enter Your Log In Data Here);
